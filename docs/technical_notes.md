...
\[
\mathbf{u}=\nabla^\perp\phi=(\partial_y\phi, -\partial_x\phi), \qquad
...
\[
E_{\mathbf{k}} = \frac{1}{2}\,|\mathbf{k}|^2\big(|\hat{\phi}_{\mathbf{k}}|^2 + |\hat{A}_{\mathbf{k}}|^2\big),
\qquad \hat{\phi}_{\mathbf{k}} = -\frac{\hat{\omega}_{\mathbf{k}}}{|\mathbf{k}|^2} \ (\mathbf{k}\neq 0).
\]
...
The solver-side diagnostics use a regularity ratio \(\rho(t)\) based on quadratic invariants built from vorticity (and, optionally, current). In the public auditor package, \(\rho\) is computed from available fields with safe guards (no log of zeros, no division by tiny denominators) and is written to `metrics.csv`.
